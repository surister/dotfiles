...
def get_connected_devices(xrandr_output, ignore_device=None):
    connected_devices = [line for line in xrandr_output.split('\n') if 'disconnected' not in line and 'connected' in line]

    if ignore_device:
        connected_devices = list(filter(lambda x: ignore_device not in x, connected_devices))
...
if __name__ == '__main__':
    ...
    connected_devices = get_connected_devices(xrandr_output, ignore_device='HDMI')
